...
function DijakstraAlgo(grid, startNode) {
	...
	queue.sort((a, b) => { return a.distance - b.distance; });
	while (queue.length > 0) {
		...
		for (let i = 0; i < neighbours.length; i++) {
			...
			if (temp < visitedNode.distance) {
				...
				if (visitedNode.getisNotVisited()) {
					...
					let node = new Node(visitedNode.row, visitedNode.col, visitedNode.width, visitedNode.total_rows);
					node.setDiscovered();
		...
		if (!currNode.getisStart()) {
			currNode.setVisited();
			let node = new Node(currNode.row, currNode.col, currNode.width, currNode.total_rows);
			node.setVisited();
			animateNodes.push(node);
		}
	}
