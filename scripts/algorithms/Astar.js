...
function AstarAlgo(ctx, grid, startNode, endNode) {
	...
	open_list.sort(function (a, b) { return a.distance - b.distance; });
	...
	while (open_list.length > 0) {
		...
		for (let i = 0; i < neighbours.length; i++) {
			...
			if (temp_score < visitedNode.g_score) {
				...
				if (visitedNode.getisNotVisited()) {
					...
					let discoverdAnimateNode = new Node(visitedNode.row, visitedNode.col, visitedNode.width, visitedNode.total_rows);
		...
		if (!currNode.getisStart()) {
			currNode.setVisited();
			let visitedAnimateNode = new Node(currNode.row, currNode.col, currNode.width, currNode.total_rows);
