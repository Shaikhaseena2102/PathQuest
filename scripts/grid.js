...
function drawWallNode(ctx, cellList, row, col) {
	if (cellList[row][col] !== undefined && cellList[row][col].getisNotVisited()) {
...
function pickRandStartAndEnd(ctx, grid) {
	...
	let startNode = notvisitedList[Math.floor(Math.random() * notvisitedList.length)];
	let endNode = notvisitedList[Math.floor(Math.random() * notvisitedList.length)];
